SCHEMA_STATEMENTS = [
    ...
    """,
    "ALTER TABLE job_invoices DROP CONSTRAINT IF EXISTS job_invoices_status_check",
    """
    ALTER TABLE job_invoices
    ADD CONSTRAINT job_invoices_status_check
    CHECK (status IN ('queued', 'running', 'finished', 'error', 'canceled'))
    """,
    "ALTER TABLE job_invoices ADD COLUMN IF NOT EXISTS matched_phrases JSONB NULL",
    "ALTER TABLE job_invoices ADD COLUMN IF NOT EXISTS pdf_url TEXT NULL",
    "ALTER TABLE job_invoices ADD COLUMN IF NOT EXISTS last_error TEXT NULL",
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS ux_job_invoices_job_invoice ON job_invoices(job_id, invoice_id)",
]
